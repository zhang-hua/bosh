    ...
    class << self
      ...
      def stop_services(attempts=20)
        retry_monit_request(attempts) do |client|
          ...
          #  client.status(group: BOSH_APP_GROUP)['hm_evacuator'][:monitor] == 0
          #end <-- This doesn't seem like the right place for this change, moving it into monit_client to be more fine-grained
        end
      end
