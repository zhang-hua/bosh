...
module Bosh::Director
  describe DeploymentPlan::UpdateStep do
    subject { DeploymentPlan::UpdateStep.new(base_job, event_log, resource_pools, assembler, deployment_plan, deployment, multi_job_updater) }
    ...
    describe '#perform' do
      ...
      it 'runs deployment plan update stages in the correct order' do
        ...
        expect(deployment_plan).to receive(:update_releases!).ordered
        expect(deployment_plan).to receive(:commit!).ordered
