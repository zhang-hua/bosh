  ...
  module Release
    class CreateRelease < Base
      ...
      def build_jobs(packages, dry_run, final)
        # provider = Bosh::Cli::ArchiveProvider.new(work_dir, :final => final, :dry_run => dry_run)
        # resources = Bosh::Cli::Resources::Job.discover(work_dir, packages)
        # artifacts = resources.map do |resource|
        #   say("Building #{resource.name.make_green}...")
        #   artifact = provider.provide(resource)
        #   nl
        #   artifact
        # end
